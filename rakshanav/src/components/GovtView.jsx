    ...
    <div style={{ display: 'flex', height: '100%', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        <div style={{
          ...
          border: `1px solid ${panelBorder}`, borderRadius: '12px', padding: '10px 14px',
            ...
            {['all', 'critical', 'high', 'medium'].map(s => {
              ...
              return (
                <button key={s} onClick={() => setFilterSeverity(s)} style={{
                  ...
                  border: `1px solid ${active ? col + '66' : (darkMode ? 'rgba(30,58,95,0.3)' : 'rgba(148,163,184,0.55)')}`,
                  background: active ? col + '20' : 'transparent',
                  color: active ? col : mutedText,
        ...
        <div style={{
          ...
          {[
            ...
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
              ...
              <span style={{ fontSize: '11px', color: bodyText }}>{label}</span>
            </div>
          ))}
          <div style={{ height: '1px', background: darkMode ? 'rgba(30,58,95,0.3)' : 'rgba(148,163,184,0.35)', margin: '8px 0' }} />
        ...
        {selectedSpot && (
          <div style={{
            ...
            background: darkMode ? 'rgba(8,12,16,0.96)' : 'rgba(255,255,255,0.97)', border: `1px solid ${severityColor[selectedSpot.severity]}44`,
            borderRadius: '14px', padding: '1.25rem', minWidth: '240px',
            boxShadow: lightShadow,
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '14px', color: titleColor }}>{selectedSpot.area}</div>
              <button onClick={() => setSelectedSpot(null)} style={{ background: 'none', border: 'none', color: mutedText, cursor: 'pointer', fontSize: '18px' }}>×</button>
            </div>
            ...
            <div style={{ fontSize: '12px', color: bodyText, marginBottom: '10px' }}>Complaints: <span style={{ color: titleColor, fontWeight: 600 }}>{selectedSpot.complaints}</span></div>
        ...
        {selectedReport && (
          <div style={{
            position: 'absolute', top: '70px', right: '10px', zIndex: 20,
            background: darkMode ? 'rgba(8,12,16,0.96)' : 'rgba(255,255,255,0.97)', border: `1px solid ${severityColor[selectedReport.severity]}55`,
            borderRadius: '14px', padding: '1.25rem', minWidth: '260px',
            boxShadow: darkMode ? `0 0 30px ${severityColor[selectedReport.severity]}22` : lightShadow,
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <div>
                ...
                <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '15px', color: titleColor }}>{selectedReport.type}</div>
              </div>
            ...
            <div style={{ fontSize: '12px', color: bodyText, marginBottom: '6px' }}>{selectedReport.description}</div>
            <div style={{ fontSize: '11px', color: subtleText, marginBottom: '12px', fontFamily: "'JetBrains Mono', monospace" }}>{selectedReport.timestamp}</div>
      ...
      <div style={{
        ...
        <div style={{ padding: '16px 20px', flex: 1 }}>
          ...
          {/* Deficit bar */}
          <div style={{ background: softCardBg, border: `1px solid ${darkMode ? 'rgba(30,58,95,0.28)' : 'rgba(203,213,225,0.9)'}`, borderRadius: '12px', padding: '14px', marginBottom: '16px', boxShadow: lightShadow }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            ...
            <div style={{ height: '7px', background: darkMode ? 'rgba(30,58,95,0.3)' : 'rgba(191,219,254,0.65)', borderRadius: '4px', overflow: 'hidden', marginBottom: '5px' }}>
              <div style={{ height: '100%', width: `${CITY_STATS.streetlightDeficit}%`, background: 'linear-gradient(90deg, #f97316, #ef4444)', borderRadius: '4px' }} />
            </div>
            <div style={{ fontSize: '11px', color: subtleText }}>Only 38% of streetlights operational in flagged wards</div>
          </div>
          ...
          {activeTab === 'reports' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {userReports.length === 0 ? (
                ...
                <div key={r.id} style={{
                  background: darkMode ? `${severityColor[r.severity]}08` : '#ffffff',
                  border: `1px solid ${darkMode ? `${severityColor[r.severity]}28` : 'rgba(226,232,240,0.95)'}`,
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '5px' }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '2px' }}>
                        ...
                        <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '13px', color: titleColor }}>{r.type}</span>
                      </div>
                      <div style={{ fontSize: '11px', color: bodyText }}>{r.description}</div>
                    </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '8px' }}>
                    <span style={{ fontSize: '10px', color: subtleText, fontFamily: "'JetBrains Mono', monospace" }}>{r.timestamp}</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {WARD_ALERTS.map((ward) => (
                <div key={ward.ward} style={{
                  background: darkMode ? `${severityColor[ward.status]}08` : '#ffffff',
                  border: `1px solid ${darkMode ? `${severityColor[ward.status]}25` : 'rgba(226,232,240,0.95)'}`,
                  borderRadius: '11px', padding: '12px 14px',
                  boxShadow: lightShadow,
                }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
                    <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '13px', color: titleColor }}>{ward.ward}</div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ fontSize: '12px', color: bodyText }}>
