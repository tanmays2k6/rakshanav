...
const path = 'e:/rakshanav-main/rakshanav-main/rakshanav/src/hooks/useGemini.js';
let c = fs.readFileSync(path, 'utf8');

const updatedUseGeminiCode = `import { useState, useCallback, useRef } from 'react';
...
export function useGeminiChat() {
  ...
  const fallbackEngine = (text) => {
    ...
    if (t.includes('route') || t.includes('navigate') || t.includes('direction')) {
      return "I can help with navigation. Let me open the Safe Route Engine for you. <action type=\\"navigate\\" target=\\"/dashboard/navigate\\" />";
    }
...
fs.writeFileSync(path, updatedUseGeminiCode, 'utf8');
console.log('Patched useGemini.js completely');
